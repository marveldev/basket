...
(function() {
  ...
  function guessCategory(name) {
    ...
    const map = {
      Produce: ['apple', 'avocado', 'banana', 'tomato', 'onion', 'potato', 'lettuce', 'spinach', 'berry', 'berries', 'lime', 'lemon'],
      ...
      Pantry: ['rice', 'pasta', 'noodle', 'flour', 'sugar', 'spice', 'oil', 'sauce', 'salt', 'pepper'],
  ...
  function renderAIHistory($aiMessages) {
    ...
    conversation.forEach(msg => {
      ...
      const bg = isUser ? 'bg-fuchsia-600 text-white rounded-tr-sm' : 'bg-gray-200 text-gray-800 rounded-tl-sm';
      $aiMessages.append(
  ...
  function initApp() {
    ...
    function renderList() {
      ...
      state.items.forEach(item => {
        ...
        const strike = item.done ? 'line-through text-gray-400' : 'text-gray-800';
        const badge = item.category ? `<span class="text-xs text-gray-400">${item.category}</span>` : '';
        $list.append(
          `<div class="item-row bg-white p-4 rounded-xl shadow-sm flex items-center gap-3 border-l-4 ${item.done ? 'border-fuchsia-500' : 'border-gray-200'}" data-id="${item.id}">` +
    ...
    function sortSmart() {
      state.items = state.items.map(item => ({ ...item, category: guessCategory(item.text) }));
    ...
    async function runAIChat(prompt) {
      ...
      try {
        await window.AppLLM.generate(prompt, {
          system: 'You are a helpful cooking assistant who answers briefly and lists ingredients clearly.',
          onToken: (t) => {
